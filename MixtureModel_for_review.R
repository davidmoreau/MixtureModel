...
plot(hist(unimod, breaks=30), col="grey",border="grey",freq=FALSE,
     xlab="Simulated d value", main="Single Gaussian")
...
plot(hist(multimod, breaks=30), col="grey",border="grey",freq=FALSE,
     xlab="Simulated d value", main="", ylim=c(0,1.09))
lines(density(multimod),lty=2, col="#FF8F00", lwd=2)
...
# Function to calculate the cumulative distribution function of a GMM
...
# Probability density function for a Gaussian mixture (mixtools structure)
...
plot(hist(es, breaks=101), col="grey", border="grey", freq=FALSE,
     xlab="Effect size (C d)", main="Effect Sizes", xlim=c(-2.5, 2.5), ylim=c(0, 1.1))
...
plot(hist(es, breaks=101), col="grey", border="grey", freq=FALSE,
     xlab="Effect size (C d)", main="3 components solution", xlim=c(-2.5, 2.5), ylim=c(0, 1.1))
lines(density(es),lty=2, col="#FF8F00")
...
# Comp estim function
comp.estim <- function (y, x = NULL, N = NULL, max.comp = 2, B = 100, sig = 0.05, 
...
theta3 <- theta4[1:3,]
theta2 <- theta4[1:2,]
mult3 <- multmixEM(watermult, lambda = rep(1, 3)/3, theta = theta2)
...
comp.estim(metadata.noNA$tau,  mix.type="normalmix")
